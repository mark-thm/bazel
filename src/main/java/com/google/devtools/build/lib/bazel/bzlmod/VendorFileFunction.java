...
public class VendorFileFunction implements SkyFunction {
  ...
  private static final String VENDOR_FILE_HEADER =
      "# This file is used to configure repositories in vendor mode, ONLY the two following functions can be used:\n" +
          "# ignore(repo_name='') is used to completely ignore this repo from vendoring.\n" +
          "# Bazel will never vendor it or consider this directory while building in vendor mode,\n" +
          "# instead it will use the normal external cache and fetch process.\n" +
          "# pin(repo_name='') is used to pin the contents of this repo under the vendor directory.\n" +
          "# Bazel will not update this repo while vendoring, and will use it as is when building in vendor mode.";
  ...
  public SkyValue compute(SkyKey skyKey, Environment env)
    ...
    RootedPath vendorFilePath =
        RootedPath.toRootedPath(
            Root.fromPath(vendorPath), LabelConstants.VENDOR_FILE_NAME);
    ...
    VendorThreadContext context = getVendorFileContext(env, skyKey, vendorFilePath.asPath());
    return VendorFileValue.create(context.getIgnoredRepos(), context.getPinnedRepos());
  }

  private VendorThreadContext getVendorFileContext(Environment env, SkyKey skyKey, Path vendorFilePath)
  ...
  private void createVendorFile(Path vendorPath, Path vendorFilePath) throws VendorFileFunctionException {
    try {
